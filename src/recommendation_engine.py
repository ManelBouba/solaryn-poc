...
def score_technology(site: pd.Series, tech: pd.Series, weather: pd.DataFrame | None=None) -> Dict:
    ...
    if not bool(perf.get("spectral_model_applied",False)):
        risks.append("No validated default spectral class is applied for this technology in the compact PoC model.")
...
def _add_scores(df: pd.DataFrame, objective: str) -> Tuple[pd.DataFrame,Dict[str,float]]:
    ...
    # Preserve the three V7 lenses without using material 'physics quality' or generic 0.75 confidence.
...
def _explain(df: pd.DataFrame) -> pd.DataFrame:
    ...
    for _,r in out.iterrows():
        ...
        if not r["spectral_model_applied"]: trade.append("Spectral differentiation is not modeled for this technology class in the compact PoC.")
...
def project_decision_summary(ranking: pd.DataFrame) -> dict:
    ...
    return {
        ...
        "scientific_status":"screening result; no probabilistic confidence claim",
    }
