...
# V7 Step 1 uses true hourly POWER data. The first three fields are GHI, DNI and DHI.
...
def fetch_nasa_power_hourly_dataframe(
    ...
    df = _parse_power_hourly_json(response.json())
...
def nasa_hourly_to_site_summary(
    ...
    """Aggregate hourly POWER data without mislabelling partial periods as annual.

    The commercial PoC requires a near-complete reference year before fields named
    ``*_year`` are populated. This prevents a month or season from being silently
    used as annual resource in lifetime calculations.
    """
    df = nasa_hourly.copy()
    if df.empty:
    ...
    unique_days = pd.Index(times.floor("D").unique())
    coverage_days = int(len(unique_days))
    expected_hours = max(coverage_days * 24, 1)
    hour_coverage = min(1.0, float(len(times)) / float(expected_hours))
    is_full_reference_year = coverage_days >= int(min_reference_days) and hour_coverage >= float(min_hour_coverage)
    if require_full_year and not is_full_reference_year:
        raise ValueError(
            "SOLARYN annual/lifetime analysis requires a near-complete reference year. "
            f"Received {coverage_days} days with {hour_coverage:.1%} hourly coverage."
        )
    ...
    ghi_period = float(ghi.fillna(0).sum() / 1000.0)
    dni_period = float(dni.fillna(0).sum() / 1000.0) if dni.notna().any() else np.nan
    rainfall_period = float(rain.fillna(0).sum())
    ...
    return pd.DataFrame([{
        ...
        "ghi_kwh_m2_year": ghi_period if is_full_reference_year else np.nan,
        "dni_kwh_m2_year": dni_period if is_full_reference_year else np.nan,
        "humidity_pct": float(humidity.mean()),
        "rainfall_mm_year": rainfall_period if is_full_reference_year else np.nan,
        ...
        "hour_coverage_fraction": round(hour_coverage, 4),
        "is_full_reference_year": bool(is_full_reference_year),
    }])
...
def fetch_nasa_power_daily_dataframe(
    ...
    """Legacy V6 daily downloader retained for compatibility, not used by the V7 app."""
...
def nasa_daily_to_site_summary(
    ...
    """Legacy V6 daily-to-site summary retained for compatibility."""
    df = nasa_daily.copy()
