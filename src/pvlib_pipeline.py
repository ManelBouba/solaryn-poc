...
def default_fixed_tilt_geometry(latitude: float, tilt_deg: float | None = None, azimuth_deg: float | None = None) -> tuple[float, float]:
    """Return a simple equator-facing fixed-tilt geometry for the PoC."""
...
def nasa_hourly_to_pvlib_weather(
    ...
    # V9 bias correction: use an anisotropic diffuse transposition model rather than
    ...
    weather = pd.DataFrame({
        ...
        "day_of_year": times.dayofyear, "hour": times.hour, "days_weight": 1.0,
