...
def matrix_path_for_module(row: pd.Series, root: str | Path | None = None) -> Path | None:
    ...
    if root is not None and not p.resolve().is_relative_to(Path(root).resolve()):
        raise ValueError("Performance matrix must be inside the project root.")
    return p
...
def spectral_evidence_policy(row: pd.Series) -> dict:
    ...
    The evidence router deliberately fails closed. A label such as ``module_specific_eqe`` is not
    ...
    The current application implements only pvlib's technology-class First Solar proxy, so
