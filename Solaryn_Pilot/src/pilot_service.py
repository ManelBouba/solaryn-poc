...
from src.lifetime_engine import add_lifetime_metrics, common_degradation_curve
...
LIMITATIONS = [
    ...
    "Common degradation and warranty are scenarios, not measured product lifetime forecasts.",
...
def analyze(root, project, modules, nasa, pvgis=None, *, store_root=None):
    ...
    weather = nasa_hourly_to_pvlib_weather(nasa, project["latitude"], project["longitude"], project["tilt_deg"], project["azimuth_deg"], albedo=project.get("albedo", .2))
    ...
    for _, module in modules.iterrows():
        ...
        candidate_results.append({"module_id": module.module_id, "manufacturer": module.manufacturer, "model": module.model, "technology": module.technology_label,
            "simulation_status": "failed" if fail else "completed", "metrics": row.iloc[0].to_dict() if len(row) else {},
            ...
            "eligibility_reasons": [fail["reason"]] if fail else [str(row.iloc[0].get("model_evidence_level", "missing"))],
            "resource_status": resource_status, "economics_status": economics["status"],
            "warnings": [fail["reason"]] if fail else [], "inputs": module.to_dict(), "parameter_provenance": [
                {"parameter": k, "value": module.get(k), "unit": unit, "source": module.get("source_url"),
                 "evidence_level": "exact_manufacturer_declared", "reviewer_state": "not_independently_reverified_in_this_release",
    ...
    result = clean({"schema_version": "2.0", "run_id": run_id, "status": "completed" if len(results) >= 2 else "failed",
                   "workspace_id": "local", "project": project, "site": {**site, "latitude": project["latitude"], "longitude": project["longitude"], "timezone": "UTC", "timezone_basis": "calculation time standard"},
