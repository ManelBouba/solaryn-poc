...
# Canonical EPC module-offer schema.  The physics engine only requires the
...
OPTIONAL_DEFAULTS = {
    ...
    "bifaciality_tolerance_pct_points": 0.0,
    "bifaciality_source": "",
}
...
_TEXT_COLUMNS = {
    ...
    "bom_source_reference", "bifaciality_source",
}
...
def normalize_module_offer_dataframe(df: pd.DataFrame) -> pd.DataFrame:
    ...
    numeric_columns = {
        ...
        "first_year_retention_pct", "annual_warranty_degradation_pct_year",
        "warranty_years", "quote_usd_w", "quote_eur_w", "thermal_u0_w_m2k", "thermal_u1_w_s_m3k",
    ...
    # Blank quotes disable economics; malformed, infinite or negative supplied quotes are errors.
    for col in ["quote_usd_w", "quote_eur_w"]:
        if col not in out:
            continue
        supplied = out[col].notna() & out[col].astype(str).str.strip().ne("")
        quote = pd.to_numeric(out[col], errors="coerce")
        if (supplied & (~np.isfinite(quote) | (quote < 0))).any():
            raise ValueError(f"{col} must be blank or a finite non-negative amount.")
        out[col] = quote
