...
from electrical_router import power, PILOT_MATRIX
import electrical_router
VERSION = 'screening-1.1.0'
STRENGTH_POLICY = 'conservative-evidence-ceiling-1.0'
...
class Configuration(Strict):
    application: Literal['GENERAL_SCREENING','ROOFTOP','UTILITY','BIPV_FACADE'] = 'GENERAL_SCREENING'
    market_region: str | None = Field(default=None, pattern=r'^[A-Z]{2}$')
...
def rank(rows, config):
    ...
    if config.economics:
        ...
        for row in rows:
            ...
            row['actual_premium_eur_w'] = econ.quotes_eur_w[mid] - econ.quotes_eur_w[econ.reference_id]
    ...
    return {'recommended_candidate': ordered[0]['module_id'] if ordered else None,
            ...
            'scientific_status': 'MODEL_BASED_PROVISIONAL',
            ...
            'strength_reason': 'Conservative evidence ceiling: common thermal, geometry and lifetime assumptions are not externally validated. A reviewed electrical route alone does not establish project validation or calibrated confidence. Strength remains Marginal under policy conservative-evidence-ceiling-1.0.'}
...
def calculate(frozen, configuration):
    ...
    for module in modules:
        ...
        pmax = module.get('rated_power_w')
        try:
            if pmax is None or not math.isfinite(pmax) or pmax <= 0:
                raise ValueError('Missing or invalid rated power')
            applications = (module.get('application') or '').split('|')
            if 'BIPV_FACADE' in applications and config.application != 'BIPV_FACADE':
                raise ValueError('Facade-only product requires a BIPV_FACADE project')
            if config.application != 'GENERAL_SCREENING' and module.get('application') and config.application not in applications:
                raise ValueError('Product application does not match the declared project')
            if config.market_region and module.get('market_regions') and config.market_region not in module['market_regions'].split('|'):
                raise ValueError('Datasheet regional scope does not match the declared project market')
            if module.get('decision_eligibility') in ('EVIDENCE_GATED','INELIGIBLE'):
                raise ValueError('Candidate lacks the minimum approved inputs for screening')
            dc, route = power(module, effective, temp, core.pvwatts_dc_power_kw_per_kwp)
            dc25, _ = power(module, effective, np.full_like(temp,25.), core.pvwatts_dc_power_kw_per_kwp)
        except (ValueError, KeyError, TypeError) as error:
            failures.append({'module_id': mid, 'reason': str(error)})
            continue
        ...
        reference25 = float(dc25.sum())
        cumulative = []
        cumulative_dc = cumulative_ac = 0.
        for year in life:
            cumulative_dc += year['dc_kwh_kwp']
            if year['ac_kwh_kwp'] is not None: cumulative_ac += year['ac_kwh_kwp']
            cumulative.append({'year':year['year'],'dc_kwh_kwp':cumulative_dc,'ac_kwh_kwp':cumulative_ac if ac is not None else None})
        rows.append({'module_id': mid, 'manufacturer': module['manufacturer'], 'model': module['model'],
                     ...
                     'annual_ac_kwh_kwp': annual_ac, 'lifetime_dc_kwh_kwp': cumulative[-1]['dc_kwh_kwp'] if cumulative else None,
                     'annual_lifetime': life, 'cumulative_lifetime':cumulative, 'actual_premium_eur_w':None, 'max_premium_eur_w': None, 'quote_eur_w': None, 'headroom_eur_w': None,
                     ...
                     **route,
                     'contributions': {'incident_reference25_kwh_kwp': float(poa.sum() / 1000),
                                       ...
                                       'electrical_response_at25_kwh_kwp': reference25 - float(effective.sum()/1000),
    ...
    if top:
        ...
        if len(top) > 1:
            ...
            explanation += f' Compared with the next ranked candidate, the modeled DC temperature contribution differs by {delta:.3f} kWh/kWp/year. Path C differentiates DC specific yield only through temperature coefficient; A/B routes use the supplied reviewed electrical response. Other layers are not inferred from family labels. Shared soiling and resource are not independent reasons to prefer a candidate.'
        if config.economics:
    ...
    versions = {'python': platform.python_version(), 'numpy': np.__version__, 'pandas': pd.__version__, 'pvlib': pvlib.__version__, 'scipy':scipy.__version__}
    code = {p.relative_to(ROOT.parent).as_posix(): hashlib.sha256(p.read_bytes()).hexdigest()
            for p in (Path(__file__), CORE, Path(electrical_router.__file__), PILOT_MATRIX)}
    drivers=[]
    if len(top)>1:
        for key,label in [('temperature_effect_kwh_kwp','Temperature response'),('electrical_response_at25_kwh_kwp','Electrical response at 25°C')]:
            if key == 'electrical_response_at25_kwh_kwp' and all(r['model_path'].startswith('C_') for r in top[:2]): continue
            drivers.append({'label':label,'value':top[0]['contributions'][key]-top[1]['contributions'][key]})
        if config.ac:
            for key in ('conversion_loss_kwh_kwp','clipping_loss_kwh_kwp','availability_curtailment_loss_kwh_kwp'):
                drivers.append({'label':key.replace('_kwh_kwp','').replace('_',' ')+' (benefit)', 'value':top[1]['contributions'][key]-top[0]['contributions'][key]})
    changer='Measured electrical response, geometry and thermal assumptions may change the ordering. No calibrated crossover threshold is available.'
    if config.economics and top:
        changer=f"Against the supplied economic reference, the leading candidate's maximum justified quote premium is {top[0]['max_premium_eur_w']:.6f} EUR/W. Above that premium its headroom relative to the reference is negative."
    visual_data={'driver_differences':drivers, 'decision_changer':changer,
                 'diagnostics':{'Mean modeled cell temperature':{'value':float(np.mean(temp)),'unit':'°C'},
                                'Maximum modeled cell temperature':{'value':float(np.max(temp)),'unit':'°C'},
                                'Hours above declared 70°C diagnostic threshold':{'value':int(np.count_nonzero(temp>70)),'unit':'hours'},
                                'Annual front POA':{'value':float(poa.sum()/1000),'unit':'kWh/m²/year'}}}
    model_paths=sorted({r['model_path'] for r in top})
    return {'decision': decision, 'visual_data':visual_data, 'candidate_failures': failures, 'explanation': explanation,
            'contributions': [{'module_id': r['module_id'], **r['contributions']} for r in top],
            'model_path': ' + '.join(model_paths) or 'NO_FEASIBLE_MODEL', 'model_version': VERSION, 'validation_status': 'PROVISIONAL_NOT_EXTERNALLY_VALIDATED',
            'extrapolation': 'SEE_CANDIDATE_DOMAIN_ACCOUNTING', 'configuration': config.model_dump(),
            'manifest': {'inputs_sha256': digest(frozen), 'configuration_sha256': digest(config.model_dump()),
                         'result_sha256': digest(decision), 'visual_data_sha256':digest(visual_data), 'strength_policy':STRENGTH_POLICY, 'code_sha256': code, 'runtime': versions},
            'implementation_sources': {p.relative_to(ROOT.parent).as_posix(): base64.b64encode(p.read_bytes()).decode('ascii')
                                       for p in (Path(__file__), CORE, Path(electrical_router.__file__), PILOT_MATRIX)},
            'assumptions': [transposition, 'Front-side DC specific energy per installed kWp; no rear, spectral or IAM differentiation; electrical off-STC response only when reviewed candidate evidence exists; no terrain/row shading.',
            ...
            'warnings': [f'Application: {config.application}; market: {config.market_region or "not declared"}. Regional restrictions and inventory require procurement verification.', 'Provisional screening recommendation. Project-level external validation remains pending. Catalog commercial status is source metadata, not a quote or inventory guarantee.',
                         'A/B routes require reviewed exact-SKU evidence; all seeded catalog rows use Path C. Datasheet fallback cannot establish cross-technology superiority. A matrix uses effective Faiman temperature as its module-temperature approximation and is not an IEC 61853-3 conformity claim.',
