...
def test_seed_modules_link_back_to_original_solaryn_database():
    ...
    assert set(modules["technology_label"]) == {"Mono PERC", "TOPCon", "HJT", "CdTe Thin Film"}
...
def test_epc_decision_refuses_winner_when_annual_and_lifetime_leaders_disagree():
    ...
    assert d["status"] == "no_robust_winner_under_declared_scenarios"
...
def test_epc_decision_accepts_stable_leader_only_with_visible_separation():
    ...
    assert d["claim_scope"] == "scenario_stability_not_probabilistic_confidence"
...
def test_switching_point_equation_uses_real_quote_and_area_bos():
    ...
    assert out.loc["ALT", "economic_model_scope"] == "module_plus_area_BOS_switching_proxy_not_LCOE"
...
def test_epc_decision_refuses_winner_when_spectral_proxy_flips_leader():
    ...
    assert d["spectral_proxy_policy"] == "must_not_change_the_robust_leader"
